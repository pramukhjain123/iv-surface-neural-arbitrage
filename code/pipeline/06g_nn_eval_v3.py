"""
Phase 3 v3 evaluation: same Task A/B RMSE + violation-rate methodology as
06c_nn_eval.py (v1), adapted for the v3 hard-calendar-constrained model
(06f_nn_train_v3.py). Calendar-arbitrage violation is expected to be ~0% at
every lambda (verified, not tuned) since it's architecturally guaranteed;
butterfly-arbitrage remains a soft, lambda-weighted penalty as in v1.
"""
import sys
...
nn_train = import_module("06f_nn_train_v3")
...
def main():
    ...
    res.to_csv(ROOT / "results" / "nn_v3_eval_summary.csv", index=False)
    grid.to_csv(ROOT / "results" / "nn_v3_violation_grid.csv", index=False)
    print(res.to_string())
