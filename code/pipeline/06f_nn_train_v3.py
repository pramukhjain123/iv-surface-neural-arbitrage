"""
Phase 3, v3: HARD-constrains calendar-arbitrage by construction instead of via a
soft penalty (v1's squared-L2 and v2's L1-with-margin penalties both failed the
same way: the optimizer could "solve" the penalty in the sparse long-tenor region
by collapsing the surface toward a flat near-zero value there, satisfying the
sign/margin check almost everywhere by not moving the slope's magnitude away from
zero, rather than by actually making it positive).

Parametrization: w(k, tau) = softplus(base(k)) + sum_i softplus(c_i(k)) * phi_i(tau)
  - phi_i are FIXED, monotonically increasing basis functions of tau alone:
        phi_1(tau) = tau
        phi_2(tau) = sqrt(tau)
        phi_3(tau) = log(1 + tau)
  - base(k) and c_1..c_3(k) come from one small MLP taking k alone (hidden layers
    softplus, output layer linear -> softplus applied per-component after).

Because every phi_i is non-decreasing in tau and every coefficient softplus(c_i(k))
is >= 0, dw/dtau = sum_i softplus(c_i(k)) * phi_i'(tau) >= 0 ALWAYS, for every k and
every tau > 0 -- calendar-arbitrage-free by construction, no training required to
enforce it and no lambda needed for it.

Butterfly arbitrage is still enforced the way v1 did (successfully): a soft L2
penalty on the Durrleman condition g(k) >= 0, evaluated via autograd on resampled
collocation points, weighted by lambda. The lambda sweep in this v3 script is a
pure butterfly-vs-fit sweep; calendar-arbitrage is uniformly ~0% at every lambda
(reported for verification, not tuned).

Usage: python3 06f_nn_train_v3.py <lambda> <seed> <n_steps_this_call> [<total_target_steps>]
Resumable like v1/v2: checkpoints in 06_nn/runs_v3/lam{lambda}_seed{seed}/ckpt.npz.
"""
import sys
import time
...
def basis(tau):
    # tau: scalar or array. Returns stacked [phi_1, phi_2, phi_3] on a new last axis.
    phi1 = tau
    phi2 = jnp.sqrt(tau)
    phi3 = jnp.log1p(tau)
...
def main():
    ...
    run_dir = ROOT / "06_nn" / "runs_v3" / f"lam{lam}_seed{seed}"
    ...
    def penalty_terms(params, ck, ctau):
        # only butterfly is a soft penalty now -- calendar is architecturally guaranteed.
